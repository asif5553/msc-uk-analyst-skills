...
Two design decisions carry the method, and both should be stated in the write-up.
  ...
  Each topic is represented by its top c-TF-IDF terms; that string is embedded with the
  same sentence encoder and compared against the 13 CATEGORY_LABELS phrases. Mapping by
  lexicon overlap would reintroduce the circularity that makes the weighted-hit TF-IDF
  variant a lexical ceiling rather than an independent method, and would invalidate the
  hard-subset comparison. No lexicon term list is used anywhere in this file.
...
def map_topics_to_categories(encoder, term_strings, topic_ids):
    """
    Cosine similarity between each topic's term string and each category label phrase.

    Returns an (n_topics, n_categories) affinity matrix. Uses CATEGORY_LABELS only -
    the same natural-language phrases the zero-shot method uses as hypotheses - so the
    mapping is independent of the annotation lexicons.
    """
    label_texts = [CATEGORY_LABELS[c] for c in CATEGORIES]
    label_emb = encoder.encode(label_texts, convert_to_numpy=True)
    topic_texts = [term_strings.get(t) or "miscellaneous" for t in topic_ids]
    topic_emb = encoder.encode(topic_texts, convert_to_numpy=True)
    ...
    affinity = unit(topic_emb) @ unit(label_emb).T

    # A topic whose c-TF-IDF representation is empty or near-empty carries no
    # interpretable content - it is usually boilerplate that survived clustering. Its
    # affinity row is zeroed so it cannot contribute to any posting's score, rather
    # than being mapped to whichever category its noise happens to lean toward.
    junk = [i for i, t in enumerate(topic_ids)
            if len([w for w in (term_strings.get(t) or "").split(",") if w.strip()]) < 3]
    if junk:
        affinity[junk] = 0.0
        print(f"  {len(junk)} topic(s) had no usable c-TF-IDF terms; excluded from mapping")
    return affinity
...
def main():
    ...
    if args.reduce_outliers and (topics == -1).any():
        ...
        print(f"  outliers reduced to {(topics == -1).mean():.1%}")
    ...
    affinity = map_topics_to_categories(encoder, term_strings, topic_ids)
    ...
    summary = {
        ...
        "n_topics": len(topic_ids), "outlier_share": outlier_share,
    ...
    if not args.no_eval:
        ...
        print("trivial hard-subset baselines: all-absent 0.453, all-present 0.547")
