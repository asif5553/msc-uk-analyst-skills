...
def evaluate_hard(y_true, y_pred, lex, categories=CATEGORIES):
    ...
    # Constant-predictor baselines. Every lexicon-FP pair has gold=0 and every
    # lexicon-FN pair has gold=1, so a constant predictor scores n_fp/n_hard
    # (always-absent) or n_fn/n_hard (always-present) on the combined subset. The
    # combined accuracy row above is therefore not evidence on its own: a method
    # demonstrates hard-subset competence only by exceeding 0.5 on BOTH error-type
    # rows, which no constant predictor can do.
    n_hard, n_fp, n_fn = int(hard.sum()), int(fp.sum()), int(fn.sum())
    if n_hard:
        rows.append({"subset": "  baseline: always-absent", "n_pairs": n_hard,
                     "accuracy": n_fp / n_hard})
        rows.append({"subset": "  baseline: always-present", "n_pairs": n_hard,
                     "accuracy": n_fn / n_hard})
