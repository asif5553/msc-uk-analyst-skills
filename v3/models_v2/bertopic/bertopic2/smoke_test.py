...
hardcsv = pd.read_csv(r / "bertopic_hard_test.csv")
assert hardcsv.subset.str.contains("always-absent").any(), "baseline rows missing"
assert hardcsv.subset.str.contains("always-present").any(), "baseline rows missing"
...
print(f"  topics               {s['n_topics']}  (outliers pre {s['outlier_share_pre_reduction']:.1%} "
      f"/ post {s['outlier_share_post_reduction']:.1%})")
